 ...
 *  Ark
 *
 *  Copyright (C) 2019 Feren OS Developera
		     ...
		     gpointer          user_data)
{
	...
	cmd = g_string_new ("ark-nemo");
	g_string_append_printf (cmd,
				" --batch --destination %s --autosubfolder -d %s",
				quoted_default_dir,
				quoted_uri);
		       ...
		       gpointer          user_data)
{
	...
	cmd = g_string_new ("ark-nemo --batch --autodestination --autosubfolder");
	      ...
	      gpointer          user_data)
{
	...
	cmd = g_string_new ("ark-nemo");
	g_string_append_printf (cmd," --add --destination %s -d", quoted_dir);
...
nemo_fr_get_name_and_desc (NemoNameAndDescProvider *provider)
{
    ...
    ret = g_list_append (ret, ("Nemo Ark:::Allows managing of archives from the context menu"));
